...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        display_content = re.sub(r'\n\n\[Qwill\'s memory note.*?\]', '', msg["content"], flags=re.DOTALL)
...
if user_input := st.chat_input("Chat with Qwill, or describe an image to create...", key="main_input"):
    ...
    if has_image:
        ...
        if is_reference_request(user_input):
            ...
            memory_note = f"\n\n[Qwill's memory note — not shown to user: the uploaded reference image showed: {description}]" if description else ""
        ...
        if final_prompt and (image_bytes_out or variations or sequence_frames):
            memory_note = f"\n\n[Qwill's memory note — not shown to user: an image was generated matching this description: {final_prompt}]"
